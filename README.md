...
`physical.location` remains construction prose and is never promoted automatically. A located spatial assertion contains one or more source-backed sensor, effect, or emitter placements. A controlled `not_applicable` assertion records why a device has no playfield point; it is not a distributed-GI/flasher exemption. Cabinet/backbox displays are not playfield devices and must use `not_applicable` with reason `cabinet_or_service`, supported by controller-topology and physical/manual provenance. Located playfield display coordinates are not supported. Author-ready v2 definitions require validated spatial evidence for every physical input/output, including individual emitter placements for lamps, GI, and flashers; every display must use the controlled non-playfield N/A rule.
