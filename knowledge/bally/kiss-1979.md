...
Kiss is a five-column game: its game-specific printed switch inventory contains exactly 40
sequential positions, 1-40, rather than column-times-ten addresses, and the retained runtime and
PinMAME model expose no game switch in positions 41-48. Public switch 16 does exist (it is Slam) -
it is the *solenoid* address 16 that does not, because on this platform the public momentary
solenoid is the PIA1:B selector plus one and selector 15 is the idle state.

The schematic-omissions supplement separately corrects the ordinary MPU J2 connector block. It
shows ST0 to ST4 on J2 pins 1 to 5 and adds missing wire number 93 at ST1, but it is not evidence
against a sixth strobe routed through another connector. Six-column BY35 machines can source ST5
elsewhere and spend a continuous output on it - on Centaur that is public 17. Kiss's own inventory
and output mapping require no ST5, so public 17 remains a real playfield output here: the right
outlane detour gate. The same public address is a strobe on one BY35 game and a coil on another;
never assume either without checking the game-specific wiring and inventory.
