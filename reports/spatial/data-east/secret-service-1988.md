...
- **controller_platform** — `controller.pinmame.dataeast`: GEN_DE is a Data East alpha platform; no reviewed pinmame.dataeast controller profile exists, so this machine must not borrow the Williams System 11 profile. Would resolve: A dedicated reviewed pinmame.dataeast profile derived from the pinned GEN_DE implementation, including its public namespace and zero invSw-mask semantics.
...
Keep partial. A reviewed Data East controller profile, mux-bank output typing, traced special-coil public mapping and timing, flipper/EOS behavior, and validated physical placements remain authoring-critical blockers.
