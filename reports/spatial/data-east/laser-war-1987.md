...
- **polarity** — `coil.driver-45`, `coil.driver-46`, `coil.driver-47`, `coil.driver-48`: The community technical chart prints the physical flipper/EOS wiring, but no retained evidence establishes which physical winding each synthetic power/hold state 45-48 stands for. Switches 46/47 are no longer part of this question: core_updateSw rewrites both from the cabinet-button bits at public 82/84 on every update, so the ROM reads them exactly as a consumer writes 82/84. Would resolve: Bench observation of the dual-winding flipper assemblies synchronized with the synthetic states 45-48.
...
Keep partial. The special-solenoid public mapping remains conflicted, and the kickback's relay-to-local-coil implementation, the winding mapping of the synthetic flipper states 45-48, and socket-level tower/GI placements remain authoring-critical blockers.
