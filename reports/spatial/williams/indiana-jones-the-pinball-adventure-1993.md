...
Status: validated. Every spatial dimension audited here is complete, and the physical machine record is `author_ready` at `machines/author-ready/williams/indiana-jones-the-pinball-adventure-1993.json`; see the promotion decision below.
...
No authoring-critical placement, quantity, or semantic question remains unresolved for the addresses this audit covers, and the deterministic curator reproduces the canonical artifact and its pinned seed byte-for-byte. The two opto-polarity conflicts the record carried are settled by the ROM itself. Switches 121-123 (Wheel Position 1-3) by its T.14 IDOL TEST (`evidence/runtime/wpc-dcs/indiana-jones-idol-test.json`), whose readback shows the ROM labelling each code the table writes with the table's own position name; switch 71 (Captive Ball Front), which the ijGameData mask leaves unnormalized unlike its column neighbors 72/73, by its T.1 SWITCH EDGES test (`evidence/runtime/wpc-dcs/indiana-jones-switch-edges.json`), where the ROM names 71 after it is set to public 1 exactly as it does the normalized optos. The `conflicts` array is empty, every coverage dimension is validated, and the record is promoted to `author_ready`.
