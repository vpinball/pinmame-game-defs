...
class AddamsFamilyDefinitionTests(unittest.TestCase):
	...
	def test_the_stale_stub_is_gone_and_gold_record_is_still_separate(self) -> None:
		...
		# identity-only partial record, the I/O attachment declared the WPC Fliptronic
		# platform from the driver's own CORE_GAMEDEF module, and the VPX script pass
		# attached candidate devices from the retained Gold script; it must never merge
		# into the curated machine.
		...
		self.assertTrue(gold_stub["inputs"] or gold_stub["outputs"])
		self.assertTrue(
			all(device["provenance"]["status"] == "candidate" for device in gold_stub["inputs"] + gold_stub["outputs"]),
			"Gold residual devices must stay candidate-only",
		)
