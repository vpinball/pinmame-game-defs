...
class SpatialMigrationTests(unittest.TestCase):
	...
	def test_all_current_author_ready_records_are_conspicuous_v2_partials_and_reports_reconcile(self) -> None:
		...
		self.assertEqual([], list((ROOT / "machines" / "author-ready").rglob("*.json")))
		...
		self.assertEqual(0, report["author_ready_count"])
		self.assertEqual(102, report["partial_count"])
		self.assertEqual(684, report["stub_count"])
		...
		self.assertEqual(0, catalog["summary"]["author_ready_count"])
		self.assertEqual(684, catalog["summary"]["stub_count"])
		self.assertEqual(103, catalog["summary"]["partial_count"])
		self.assertEqual(1, catalog["summary"]["non_game_count"])
		note_paths = {definition["knowledge"]["path"] for definition in migrated.values()}
		self.assertEqual(33, len(note_paths))
