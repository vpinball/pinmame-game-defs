...
class SpatialMigrationTests(unittest.TestCase):
	...
	def test_pending_spatial_retrofits_are_conspicuous_v2_partials_and_reports_reconcile(self) -> None:
		...
		self.assertEqual(29, len(migrated))
		...
		self.assertEqual(4, len(list((ROOT / "machines" / "author-ready").rglob("*.json"))))
		...
		self.assertEqual(4, report["author_ready_count"])
		...
		self.assertEqual(29, report["missing_requirement_counts"]["spatial_placement"])
		...
		self.assertEqual(4, catalog["summary"]["author_ready_count"])
		...
		self.assertEqual(29, len(note_paths))
