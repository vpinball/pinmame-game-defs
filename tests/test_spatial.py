...
class SpatialMigrationTests(unittest.TestCase):
	...
	def test_pending_spatial_retrofits_are_conspicuous_v2_partials_and_reports_reconcile(self) -> None:
		...
		self.assertEqual(24, len(migrated))
		...
		self.assertEqual(9, len(list((ROOT / "machines" / "author-ready").rglob("*.json"))))
		...
		self.assertEqual(9, report["author_ready_count"])
		...
		self.assertEqual(9, catalog["summary"]["author_ready_count"])
		...
		self.assertEqual(24, len(note_paths))
