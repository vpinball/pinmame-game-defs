...
class SpatialMigrationTests(unittest.TestCase):
	...
	def test_pending_spatial_retrofits_are_conspicuous_v2_partials_and_reports_reconcile(self) -> None:
		...
		# The 2026-08-28 Junk Yard curation and the Big Buck Hunter Pro pass each replace one
		# stub with an honest partial, so the physical partial count rose by two and the stub
		# count fell by two over the 2026-08-20 handoff baseline.
		self.assertEqual(98, report["partial_count"])
		self.assertEqual(666, report["stub_count"])
		...
		# The 2026-08-28 Junk Yard curation and the Big Buck Hunter Pro pass each replace one
		# stub with an honest partial and each add one spatial gap, so both counts rise by two
		# over the pre-curation baseline of 44.
		...
		self.assertEqual(52, report["missing_requirement_counts"]["unresolved_conflicts"])
		...
		self.assertEqual(666, catalog["summary"]["stub_count"])
		# The catalog count includes the separately classified partial diagnostic; coverage counts
		# only the 789 physical games and therefore reports 98 partial records above.
		self.assertEqual(99, catalog["summary"]["partial_count"])
