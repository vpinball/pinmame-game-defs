...
class SpatialMigrationTests(unittest.TestCase):
	...
	def test_pending_spatial_retrofits_are_conspicuous_v2_partials_and_reports_reconcile(self) -> None:
		...
		self.assertEqual(23, len(migrated))
		...
		self.assertEqual(10, len(list((ROOT / "machines" / "author-ready").rglob("*.json"))))
		...
		self.assertEqual(10, report["author_ready_count"])
		...
		self.assertEqual(10, catalog["summary"]["author_ready_count"])
		...
		self.assertEqual(23, len(note_paths))
