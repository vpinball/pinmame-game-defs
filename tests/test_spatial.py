...
class SpatialMigrationTests(unittest.TestCase):
	...
	def test_pending_spatial_retrofits_are_conspicuous_v2_partials_and_reports_reconcile(self) -> None:
		...
		self.assertEqual(31, len(migrated))
		...
		self.assertEqual(2, len(list((ROOT / "machines" / "author-ready").rglob("*.json"))))
		...
		self.assertEqual(2, report["author_ready_count"])
		self.assertEqual(100, report["partial_count"])
