...
class IdentityTests(unittest.TestCase):
	...
	def test_the_superseded_stub_still_claims_only_the_stub_machine_id(self) -> None:
		"""This worktree must not regenerate the shared catalog, so the stub it supersedes stays on disk.

		Deleting it here would leave catalog/pinmame.json pointing at a missing file. The integrator's
		rebuild_catalog call prunes the stub definition and its knowledge note once this definition
		claims the potc_* clone-tree root. What this test does guard is that the stub has not been
		edited to claim the physical machine id, which would make two records fight over it.
		"""
		self.assertTrue(SUPERSEDED_STUB_PATH.is_file())
		self.assertTrue(SUPERSEDED_STUB_KNOWLEDGE_PATH.is_file())
		stub = load_json(SUPERSEDED_STUB_PATH)
		self.assertEqual("stub.pinmame.potc_600af", stub["machine"]["id"])
		self.assertEqual("stub", stub["coverage"]["status"])
		self.assertNotEqual(MACHINE_ID, stub["machine"]["id"])
