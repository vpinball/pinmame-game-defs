...
class LordOfTheRingsDefinitionTests(unittest.TestCase):
	...
	def test_promotion_state_is_held_at_partial(self) -> None:
		"""Held at partial by two gaps.

		The 520-5242-00 LED board at public lamps 81-99 is not enumerated, and switch 15's
		public opto polarity is unestablished.
		"""
		...
		self.assertEqual("observed", coverage["dimensions"]["address_enumeration"])
	...
	def test_led_board_gap_is_declared_not_hidden(self) -> None:
		report = load_json(SPATIAL_REPORT_PATH)
		self.assertEqual("pinmame-spatial-blockers", report["format"])
		blocker = next(item for item in report["blockers"] if item["id"] == "led-board-520-5242-00-not-enumerated")
		self.assertEqual("major", blocker["severity"])
		self.assertIn("81-99", blocker["summary"])
		self.assertIn("SE_BOARDID_520_5242_00", blocker["evidence"])
		self.assertIn("whitestar.json", blocker["why_not_fixed_here"])
	...
	def test_pop_bumper_flasher_is_three_bulbs_with_one_placement(self) -> None:
		flasher = self.solenoids[25]
		self.assertEqual(3, flasher["physical"]["quantity"])
		self.assertEqual(1, len(flasher["spatial"]["placements"]))
	...
	def test_lamp_enumeration_is_exactly_one_to_eighty(self) -> None:
		"""The printed 8x10 matrix is public 1-80. The nineteen LED-board lamps at 81-99 are real but
		cannot be bound while the shared Whitestar profile caps lamps at 80, so they are declared
		missing rather than enumerated here."""
...
class LordOfTheRingsRetainedEvidenceTests(unittest.TestCase):
    ...
    def test_every_external_locator_names_a_file_that_is_there(self) -> None:
        ...
        for source in self.definition["sources"]:
            ...
            checked += 1
        self.assertEqual(12, checked, "every retained artifact must carry an external locator")
    ...
    def test_extraction_manifests_recompute(self) -> None:
        ...
        # got a mismatch.
        for source_id in ("vpx-extraction.lotr-vpw-1-6", "vpx-extraction.lotr-jpsalas-600", "vpx-extraction.lotr-hanibal-4k"):
            path = _resolve_external(self.sources[source_id]["uri"])
            self.assertTrue(path.is_file(), f"{source_id} locator resolves to nothing: {path}")
