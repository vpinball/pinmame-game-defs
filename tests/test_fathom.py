...
class FathomIdentityTests(unittest.TestCase):
	...
	def test_superseded_stub_is_still_present_for_the_integrator(self) -> None:
		"""This worktree deliberately does not regenerate the catalog, so the stub is pruned centrally."""
		self.assertTrue(STUB_PATH.is_file())
		self.assertEqual("stub.pinmame.fathom", load_json(STUB_PATH)["machine"]["id"])
