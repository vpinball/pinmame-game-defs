...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 785 physical-game records plus one explicitly classified non-game diagnostic. AC/DC Vault, Spider-Man Vault, original Spider-Man (Stern 2007), AC/DC LED Pro, The Walking Dead Premium/LE, The Walking Dead Pro, Iron Man Pro Vault, Mustang Premium/LE/Boss, Mustang Pro, Star Trek Premium/LE, Star Trek Pro, Metallica Premium/LE, Metallica Pro, original AC/DC Pro, AC/DC Premium/LE/LUCI, Avengers Pro, Rolling Stones Standard, Rolling Stones Limited Edition, TRON: Legacy Limited Edition, and Avatar LE are complete in the current tree. The regenerated catalog yields 19/785 author-ready games, 83 partial games, 683 conspicuous game stubs, and 15 records that explicitly report missing spatial placement; Scott's diagnostic is separately partial and excluded from the denominator. Terminator 2 now has a complete controller inventory, semantic mapping, retained recreation knowledge, and source-backed spatial evidence, but remains one of the partials because author-critical conflicts are unresolved. |
...
| Static catalog site | In progress | Copy the existing external `pinmame-games` source into `site/` without build caches or generated output, install its Pages workflow at repository root, configure the custom-domain build for `games.visualpinball.org`, build and verify it against the combined catalog, obtain separate Opus approval, commit on local `master`, push `master`, verify deployment, and report the exact DNS target. |
| Continuation runbook | In progress | `INSTRUCTIONS.md` exists in an isolated worktree and covers the full evidence, model-routing, per-game, Opus-review, integration, cleanup, publication, harness, and Ghidra workflow. It is currently unlocked for user improvements; after the user finishes, it will receive its own validation, Opus review, commit, integration review, and cleanup. |
