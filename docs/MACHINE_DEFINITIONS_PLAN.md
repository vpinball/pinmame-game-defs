...
Status: Implementation in progress (2026-08-03, 55%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The physical-only cleanup was approved and committed at `e53d7c7`, the Ali physical-family correction at `59bf6bb`, and the first complete spatial retrofit—AC/DC Vault Edition—at `252c740`. Spider-Man Vault Edition (2016) is implemented and locally validated as the second author-ready spatial definition, pending the required fresh Opus review and per-game commit. The remaining 31 semantic baselines stay fail-closed until each has reviewed coordinate evidence; Terminator 2 remains deferred until all 33 spatial retrofits are complete.
...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 785 physical-game records plus one explicitly classified non-game diagnostic. AC/DC Vault Edition is committed. Spider-Man Vault Edition now has all 96 inputs and 126 outputs spatially dispositioned, including 32 direct playfield GI bulbs, ten projected rear-panel GI bulbs, and two documented coin-door bulbs without playfield coordinates. The locally regenerated catalog yields 2/785 author-ready games, 100 partial games, 683 conspicuous game stubs, and 31 remaining spatial retrofits; Spider-Man remains uncommitted until Opus approval, and Scott's diagnostic is separately partial and excluded from the denominator. |
...
| Validation and Opus review | In progress | The physical-only cleanup, Ali correction, and AC/DC Vault spatial retrofit were approved and committed. AC/DC Vault passed 231 full-suite tests, repository validation, deterministic regeneration, and the spatial-overlay audit; its correction round returned exact `APPROVED`. Spider-Man Vault passed 238 full-suite tests, repository validation, deterministic regeneration, and overlay inspection; its final Opus documentation correction is in progress before commit. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, and DMD-capable reusable LibPinMAME service/gameplay harness are implemented. The Ali identity correction and AC/DC Vault spatial retrofit are committed. Spider-Man Vault passed the full suite, repository validation, deterministic regeneration, and overlay inspection; its first Opus pass found evidence-boundary issues in GI projection and Green Goblin hardware treatment, and the correction round is in progress. The other 31 semantic baselines remain fail-closed for spatial placement. The next work package is ordered as follows:

1. Complete Spider-Man Vault's Opus correction round, rerun the full-suite, repository, deterministic-regeneration, and overlay checks on the corrected artifact, obtain exact approval, then commit it as the second spatially author-ready game.
2. Process the remaining spatial records newest physical release first, preserving fail-closed status until each title has reviewed coordinate evidence and a fresh Opus-reviewed commit; process Ali's spatial work later at 1980, and finish all 33 before Terminator 2.
