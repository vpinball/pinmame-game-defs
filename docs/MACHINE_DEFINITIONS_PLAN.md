...
Status: Implementation in progress (2026-08-03, 55%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The physical-only cleanup was approved and committed at `e53d7c7`, and the Ali physical-family correction was approved and committed at `59bf6bb`. AC/DC Vault Edition is the first complete spatial retrofit: implementation, full validation, and the correction-round Opus review are complete, and the approved per-game commit is next. The remaining 32 semantic baselines stay fail-closed until each has reviewed coordinate evidence; Terminator 2 remains deferred until all 33 spatial retrofits are complete.
...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 785 physical-game records plus one explicitly classified non-game diagnostic. AC/DC Vault Edition now has all 96 inputs and 117 outputs spatially dispositioned: 139 located devices with 185 sensor/effect/emitter placements and 74 controlled N/A records. This yields 1/785 author-ready games, 101 partial games, 683 conspicuous game stubs, and 32 remaining spatial retrofits; Scott's diagnostic is separately partial and excluded from the denominator. |
...
| Validation and Opus review | In progress | The physical-only cleanup and Ali correction were approved and committed. AC/DC Vault passed 231 full-suite tests, repository validation, deterministic regeneration, and the spatial-overlay audit. Its first fresh Opus review found two semantic label corrections plus three fail-closed/documentation improvements; all five were resolved and the correction round returned exact `APPROVED`. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, and DMD-capable reusable LibPinMAME service/gameplay harness are implemented. The Ali identity correction is committed. AC/DC Vault spatial implementation is complete, validated, and Opus-approved for commit; the other 32 semantic baselines remain fail-closed for spatial placement. The next work package is ordered as follows:

1. Commit the approved AC/DC Vault diff as the first spatially author-ready game.
