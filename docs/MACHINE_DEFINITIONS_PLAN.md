...
Status: Implementation in progress (2026-08-03, 55%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The physical-only cleanup was approved and committed at `e53d7c7`, the Ali physical-family correction at `59bf6bb`, AC/DC Vault Edition at `252c740`, Spider-Man Vault Edition at `c4568f5`, and the Simpsons Pinball Party IPDB identity correction at `441527b`. AC/DC LED Pro is implemented locally as the third author-ready spatial definition and awaits its required Opus review and per-game commit. The remaining 30 semantic baselines stay fail-closed until each has reviewed coordinate evidence; Terminator 2 remains deferred until all 33 spatial retrofits are complete.
...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 785 physical-game records plus one explicitly classified non-game diagnostic. AC/DC Vault and Spider-Man Vault are committed as the first two spatially author-ready games, and AC/DC LED Pro is the locally implemented third. The regenerated catalog yields 3/785 author-ready games, 99 partial games, 683 conspicuous game stubs, and 30 remaining spatial retrofits; Scott's diagnostic is separately partial and excluded from the denominator. |
...
| Validation and Opus review | In progress | The physical-only cleanup, Ali correction, AC/DC Vault, Spider-Man Vault, and TSPP identity correction were approved and committed. AC/DC LED Pro passes 242 full-suite tests, repository validation, deterministic regeneration, exact spatial totals, and overlay inspection; it awaits its fresh Opus review before commit. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, and DMD-capable reusable LibPinMAME service/gameplay harness are implemented. The Ali identity correction plus AC/DC Vault and Spider-Man Vault spatial retrofits are committed. TSPP's incorrect IPDB 6154 identity was traced to a bad third-party VPX header, corrected to 4674, and committed at `441527b` without altering pinned/generated third-party evidence. AC/DC LED Pro (2014) is implemented locally as an author-ready spatial promotion in this worktree and remains pending review; the other 30 semantic baselines remain fail-closed for spatial placement. The next work package is ordered as follows:

1. Review the local AC/DC LED Pro (2014) spatial promotion and obtain its fresh approval before committing it; do not treat this local implementation as approved yet.
2. Continue newest physical release first after LED Pro; preserve fail-closed status until each title has reviewed coordinate evidence and a fresh Opus-reviewed commit, process Ali's spatial work later at 1980, and finish all 30 remaining pending records before Terminator 2.
