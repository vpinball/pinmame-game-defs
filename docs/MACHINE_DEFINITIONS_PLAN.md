...
Status: Implementation in progress (2026-08-03, 50%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; Rolling Stones Standard/LE and TRON: Legacy Pro/LE received the same explicit approval. Avatar Pro/LE is implemented and locally validated with corrected DMD-capable runtime evidence. Its first persistent-session Opus pass withheld approval over one blocker and four majors; after literal service commands/DMD/output observations, LE-only provenance, edition-conditional behavior, undocumented power-feed handling, and manual-proven lamp-matrix orientation were corrected, the full re-review returned `APPROVED: safe to commit`.
...
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 786 game records plus one explicitly classified non-game diagnostic after reviewed physical/virtual product splits. Current author-ready game coverage is 29/786 (3.6896%), with 71 partial games and 686 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. Avatar Pro and Limited Edition now exhaustively split all five `avr_` drivers, increasing the physical-game denominator by one because the previous mixed clone-tree stub represented two incompatible products. TRON: Legacy Pro/LE, Rolling Stones Standard/LE, Transformers, X-Men, Star Trek, Mustang, Walking Dead, Avengers, Metallica, all five AC/DC products, Spider-Man 2007/Vault, Ali seven-digit conversion, and Cheech & Chong remain author-ready. Iron Man Vault Edition is the next newest unresolved record. |
| Semantic evidence import | In progress | Existing C#/JSON data, 55 PinMAME simulations, and 1,500 scripts from the two pinned VPX corpora have been extracted with provenance. A known-working VPX script has encoded precedence for runtime addresses, callbacks, ball routing, initial state, and mechanism causality; manuals remain authoritative for physical construction and PinMAME for emulator metadata. The isolated LibPinMAME harness now captures segment text plus raw DMD pixel hashes/optional grayscale artifacts and snapshots solenoids from callback state rather than the legacy SAM query alias. Avatar combines the organized official 55-page scanned manual, exact `avr_200` working script, authenticated exact Pro VPX, exact Pro/LE localized ROM tables, an authenticated real-machine LE diagnostic dump, IPDB 5618/5653 identities, corrected `avr_200`/`avr_120h` boot-start traces, and exact-ROM service tests for the three-bank, suit motor, and both transporter directions. This resolves the complete matrix/dedicated/DIP inputs, Q1-Q32, game-on 33, unused SAM auxiliary capacity 51-66, lamp 1-80, GI 0, DMD, Pro/LE insert and switch deltas, four-ball ceramic identity, AMP mechanisms, separate Link lock and transporter causality, and marching legs. Earlier curated products retain their documented source sets and exact physical boundaries. |
| Recreation knowledge | In progress | A conspicuous stub note exists for every unresolved root. Complete source-linked notes now additionally cover Avatar Pro/LE. They preserve the four-ball rather than five-ball LE inventory, per-ball ceramic material identity and metal-rail detector behavior, AMP bank travel/collision gating, suit motor direction and endpoints, alternating marching legs, the non-centering magnet, Link lock pin/latch causality, one normally-closed captive ball, and the separate single-motor reversing transporter with both endpoints. The notes explicitly exclude LE-only switches, outputs, lamps, and mechanisms from Pro and keep PinMAME's unused auxiliary address capacity nonphysical. Earlier detailed product notes remain unchanged. |
...
| Validation and Opus review | In progress | Repository validation passes with exact definition/catalog driver membership and stronger author-ready gates; all 172 regression tests pass. Twelve Avatar fixtures lock the physical model split, complete address spaces, contact polarity, wiring, exact evidence hashes, ceramic detector, AMP mechanisms, both transporter directions, and single-owner actuator semantics. Twelve harness fixtures cover DMD pixel capture, bounded DMD event retention, and callback-based SAM snapshot correctness. The persistent Opus session approved the committed foundation, Rolling Stones, and TRON checkpoints. Its first Avatar pass withheld approval over one blocker, four majors, and four minors; every blocker/major was corrected, all actionable minors were fixed or reconciled directly against manual pages 36/41, and the complete re-review returned `APPROVED: safe to commit`. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers, eight omitted abstract parent containers, and 773 clone-tree roots currently resolved to 786 game records plus one non-game diagnostic after reviewed product splits, including separate Avatar Pro/LE and TRON Pro/LE records. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, DMD-capable reusable LibPinMAME service/gameplay harness, and first 29 author-ready records are implemented. Work continues newest-to-oldest without awarding completion credit to stubs or partial records:
...
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, both Transformers editions, Rolling Stones Standard/LE, TRON: Legacy Pro/LE, and Avatar Pro/LE are complete. Commit the Opus-approved Avatar batch, then process the existing Iron Man Vault Edition partial and continue backward while grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly before curation.
