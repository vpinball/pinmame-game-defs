...
Status: Implementation in progress (2026-08-03, 55%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The physical-only cleanup was approved and committed at `e53d7c7`. Spatial-placement infrastructure intentionally fail-closes the 33 semantic baselines until each has reviewed coordinate evidence and receives a fresh Opus review; Terminator 2 remains deferred until all 33 are spatially complete.
...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 786 physical-game records plus one explicitly classified non-game diagnostic. Spatial migration deliberately changes the 33 current physical author-ready records to schema-v2 partials with `spatial_placement` missing and unknown, yielding 0/786 completion credit, 102 partial games, and 684 conspicuous game stubs; Scott's diagnostic is separately partial and excluded from the denominator. Existing semantic data and knowledge completeness are retained. |
...
| Validation and Opus review | In progress | The physical-only cleanup was approved and committed at `e53d7c7`. Spatial infrastructure has 223 green tests and is pending fresh Opus review; spatial approval has not yet been granted. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, and DMD-capable reusable LibPinMAME service/gameplay harness are implemented. The 33 semantic baselines are fail-closed for spatial placement, and the next work package is ordered as follows:

1. Complete the Ali identity correction as its own immediate cleanup, with explicitly labeled owner-built/community seven-digit conversion variants; do not combine that identity change with spatial retrofit.
2. Process the spatial records newest physical release first, preserving the fail-closed status until each title has reviewed coordinate evidence and a fresh Opus-reviewed commit; process Ali's spatial work later at 1980, and finish all 33 before Terminator 2.
...
6. Keep schema validation, exact catalog reachability, coverage reporting, and regression tests green after every curation batch; spatial approval remains pending fresh Opus review.
