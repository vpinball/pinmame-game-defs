...
Status: Implementation in progress (2026-08-03, 51%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; Rolling Stones Standard/LE, TRON: Legacy Pro/LE, and Avatar Pro/LE received the same explicit approval. Iron Man 2010 and Iron Man Pro Vault Edition 2014 are implemented and locally validated as separate physical products. Their first persistent-session Opus pass withheld approval over three majors; after sustained-contact polarity, the Q24 script/manual conflict, the Q8 table-versus-schematic voltage discrepancy, and every actionable minor were corrected, the complete re-review returned `APPROVED: safe to commit`.
...
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 787 game records plus one explicitly classified non-game diagnostic after reviewed physical/virtual product splits. Current author-ready game coverage is 31/787 (3.9390%), with 70 partial games and 686 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. Iron Man now exhaustively splits all thirteen `im_` drivers into ten original-product and three Vault-product revisions, increasing the denominator because the previous mixed partial represented two physical products. Avatar Pro/LE, TRON: Legacy Pro/LE, Rolling Stones Standard/LE, Transformers, X-Men, Star Trek, Mustang, Walking Dead, Avengers, Metallica, all five AC/DC products, Spider-Man 2007/Vault, Ali seven-digit conversion, and Cheech & Chong remain author-ready. Stern 24 is the next newest unresolved record. |
| Semantic evidence import | In progress | Existing C#/JSON data, 55 PinMAME simulations, and 1,500 scripts from the two pinned VPX corpora have been extracted with provenance. A known-working VPX script has encoded precedence for runtime addresses, callbacks, ball routing, initial state, and mechanism causality; manuals remain authoritative for physical construction and PinMAME for emulator metadata. The isolated LibPinMAME harness captures segment text plus raw DMD pixel hashes/optional grayscale artifacts and snapshots solenoids from callback state rather than the legacy SAM query alias. Iron Man combines organized official 2010 parts and 2014 service manuals, pinned exact `im_185ve` VPW scripts from both corpora, IPDB 5550/6154 identities, Stern's official reissue/construction boundary and shared-code notice, exact ROM archive/member hashes, and a debounced exact-ROM boot/start trace. This resolves every matrix/dedicated/DIP input, Q1-Q32, game-on 33, unused SAM auxiliary capacity 51-66, lamp 1-80 including unlabeled row-10 #555 load bulbs 73/79/80, GI 0, DMD, four-ball path, two magnets, War Machine kickback, both up-posts, and Iron Monger travel/collision/endpoint causality without projecting the Vault LED/toy/backbox construction onto the original. Earlier curated products retain their documented source sets and exact physical boundaries. |
| Recreation knowledge | In progress | A conspicuous stub note exists for every unresolved root. Complete source-linked notes now additionally cover Iron Man 2010 and Iron Man Pro Vault Edition 2014. They preserve the four-ball trough/shooter route, physical Tournament Start at switch 15 despite a VPX bookkeeping pulse, War Machine capture/kickback, two non-centering magnets, unsensed orbit/center posts, motorized Iron Monger target/collision motion and real endpoint timing, original incandescent versus Vault LED/flasher construction, the strengthened one-piece Vault toys/launcher, and the cabinet/backbox boundary. Earlier detailed product notes remain unchanged. |
...
| Validation and Opus review | In progress | Repository validation passes with exact definition/catalog driver membership and stronger author-ready gates; all 183 regression tests pass. Eleven focused Iron Man fixtures lock the product split, complete input/output/lamp spaces, sustained contacts, corrected physical labels, Q24 conflict disposition, schematic-proven 16 VAC shaker wiring, original/Vault lighting boundary, exact source/runtime hashes, custom mechanism causality, and single-owner actuators. The first Iron Man Opus pass withheld approval over three majors and seven minors; all three majors and every actionable minor were remediated, and the same persistent session returned `APPROVED: safe to commit` after verifying generated hashes and the full amended diff. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, DMD-capable reusable LibPinMAME service/gameplay harness, and first 31 author-ready records are implemented. Work continues newest-to-oldest without awarding completion credit to stubs or partial records:
...
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, both Transformers editions, Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, and both Iron Man physical products are complete locally. Commit the Opus-approved Iron Man batch, then process Stern 24 and continue backward while grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly before curation.
