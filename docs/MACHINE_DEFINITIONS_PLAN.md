...
Status: Implementation in progress (2026-08-03, 55%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The physical-only cleanup was approved and committed at `e53d7c7`, the Ali physical-family correction at `59bf6bb`, AC/DC Vault Edition at `252c740`, and Spider-Man Vault Edition at `c4568f5`. The incorrect Simpsons Pinball Party IPDB identity propagated from a bad VPX header is being corrected from Iron Man Vault ID 6154 to TSPP ID 4674 with explicit regression coverage. The remaining 31 semantic baselines stay fail-closed until each has reviewed coordinate evidence; Terminator 2 remains deferred until all 33 spatial retrofits are complete.
...
| Validation and Opus review | In progress | The physical-only cleanup, Ali correction, AC/DC Vault, and Spider-Man Vault spatial retrofit were approved and committed. Spider-Man passed 238 full-suite tests, repository validation, deterministic regeneration, and overlay inspection; its final correction round returned exact `APPROVED`. The TSPP IPDB correction is implemented locally and awaits its own required Opus review before commit. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, and DMD-capable reusable LibPinMAME service/gameplay harness are implemented. The Ali identity correction plus AC/DC Vault and Spider-Man Vault spatial retrofits are committed. TSPP's incorrect IPDB 6154 identity has been traced to a bad third-party VPX header and corrected locally to 4674 without altering pinned/generated third-party evidence. The other 31 semantic baselines remain fail-closed for spatial placement. The next work package is ordered as follows:

1. Validate and obtain Opus approval for the isolated TSPP IPDB correction, then commit the legacy identity input, canonical record, catalog hash changes, plan status, and regression test together.
2. Promote AC/DC LED Pro (2014) as the next spatial record, then continue newest physical release first; preserve fail-closed status until each title has reviewed coordinate evidence and a fresh Opus-reviewed commit, process Ali's spatial work later at 1980, and finish all 33 before Terminator 2.
