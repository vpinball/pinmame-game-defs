...
Status: Implementation in progress (2026-08-02, 49%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; the corrected Rolling Stones Standard/LE delta received the same explicit approval. TRON: Legacy Pro/LE is implemented and locally validated; the first persistent-session pre-commit pass found five substantive and four minor issues, all nine were corrected, and the evidence-complete re-review returned `APPROVED: safe to commit`.
...
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 785 game records plus one explicitly classified non-game diagnostic after reviewed physical/virtual product splits. Current author-ready game coverage is 27/785 (3.4395%), with 71 partial games and 687 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. TRON: Legacy Pro and Limited Edition now exhaustively split all 13 `trn_` drivers with incompatible target, Recognizer, lamp, dome, ramp-RGB, and dedicated-flipper-input hardware kept separate. Rolling Stones Standard/LE, Transformers, X-Men, Star Trek, Mustang, Walking Dead, Avengers, Metallica, all five AC/DC products, Spider-Man 2007/Vault, Ali seven-digit conversion, and Cheech & Chong remain author-ready. Avatar Limited Edition is the next newest unresolved game. |
| Semantic evidence import | In progress | Existing C#/JSON data, 55 PinMAME simulations, and 1,500 scripts from the two pinned VPX corpora have been extracted with provenance. A known-working VPX script has encoded precedence for runtime addresses, callbacks, ball routing, initial state, and mechanism causality; manuals remain authoritative for physical construction and PinMAME for emulator metadata. A reusable isolated LibPinMAME harness captures switch injection, displays, output transitions, snapshots, and exact ROM/run hashes. TRON combines an organized official 111-page Pro/LE manual, exact `trn_174h` working script, pinned `SAM_5COL+1` tricolor serialization, exact IPDB Pro 5682/LE 5707 identities, and exact `trn_174`/`trn_174h` boot-start traces with the common target bank initialized down on switch 52. This resolves both complete 64-switch/80-lamp/32-physical-output charts, synthetic game-on 33, explicit unused output-34 VPX compatibility artifact, public RGB 101-106, BGR channel order, Pro fixed TRON bank versus LE four-drop reset, common moving three-target bank, LE three-position moving Recognizer, three-output disc causality, orbit post, and the dedicated D13/D14 split proven by Pro PDF page 49 versus LE PDF page 55. Rolling Stones, Transformers, AC/DC, Metallica, Star Trek, X-Men, Mustang, Avengers, and Walking Dead retain the previously documented organized manuals, model-specific scripts, runtime traces, public-address translations, and exact product boundaries. |
| Recreation knowledge | In progress | A conspicuous stub note exists for every unresolved root. Complete source-linked notes now cover all five AC/DC products, both Metallica, Spider-Man, Star Trek, Mustang, Walking Dead, Avengers, X-Men, Transformers, Rolling Stones, and TRON products, plus the Ali conversion and Cheech & Chong. The TRON notes preserve the four-ball startup, launcher/eject constants, independent three-target bank and disc physics, orbit post, three-flipper topology, complete routes/targets/lighting, and exact Pro/LE exclusions. The LE note additionally preserves four resettable TRON drops, ±20-degree Recognizer sweep with three narrow sensor windows, BGR ramp triples, the resolved manual-versus-working-script dome-side disagreement, and the nonphysical commented output-34 artifact; the Pro note preserves fixed standups, page-located upper-flipper button/EOS, incandescent/LED split, and absence of LE motion/RGB hardware. Existing detailed product notes remain unchanged. |
...
| Validation and Opus review | In progress | Repository validation passes with exact definition/catalog driver membership and stronger author-ready gates; all 156 regression tests pass, including nine TRON fixtures for full address spaces, exact model split and IPDB identities, corrected pulse semantics, custom-mechanism ownership, BGR public ordering, staged upper-flipper differences, output-34 nonphysical compatibility handling, and immutable source/runtime hashes. The persistent Opus session approved the committed foundation and Rolling Stones checkpoint. Its first TRON pass withheld approval over nine findings; after correction it independently verified the manual, VPX, PinMAME, and raw-trace evidence and returned `APPROVED: safe to commit`. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers, eight omitted abstract parent containers, and 773 clone-tree roots currently resolved to 785 game records plus one non-game diagnostic after reviewed product splits, including the new separate TRON Pro/LE records. |
