...
Status: Implementation in progress (2026-08-02, 47%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit.
...
| Validation and Opus review | In progress | Repository validation passes with exact definition/catalog driver membership and stronger author-ready gates; 136 evidence-policy, manual-cache/extraction, ROM-index, classification, runtime-harness, AC/DC, Metallica, Avengers, Ali, Cheech & Chong, Walking Dead, Mustang, Star Trek, X-Men, and Transformers regression tests pass. Scanned-manual detection ignores whitespace and page-number-only text layers; game/non-game accounting, edition separation, physical-model identity, auxiliary-board remapping, exact source hashes, controller-plugin group IDs, WPC public address ranges, trough topology, service-output translations, discrete SCR address holes, custom mechanisms, actuator ownership, physical connector uniqueness, runtime-observation reconciliation, SAM game-on semantics, manual-download integrity, target/rollover mappings, extended-lamp mappings, complete AC/DC, Metallica, Star Trek, X-Men, and Transformers address spaces, product-specific lamp/GI/mechanism deltas, inherited virtual-retheme contracts, and ROM/script activity that must not invent physical outputs are protected by fixtures. The persistent Opus session completed two full review passes plus a final verification and explicitly approved the corrected tree as safe to commit. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers, eight omitted abstract parent containers, and 773 clone-tree roots currently resolved to 783 game records plus one non-game diagnostic after reviewed Transformers, X-Men, Spider-Man, The Walking Dead, Mustang, Star Trek, Avengers, AC/DC, and Metallica product splits and the Ali conversion-family merge. |
...
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, and both Transformers editions are complete. Process Rolling Stones Pro/LE next, followed by the existing Tron Legacy LE partial plus its Pro sibling, then continue backward while grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly before curation.
