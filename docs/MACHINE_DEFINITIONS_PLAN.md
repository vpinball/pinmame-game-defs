...
Status: Implementation in progress (2026-08-03, 53%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, both Iron Man physical products, Stern 24, and Batman: The Dark Knight received the same explicit approval. Batman is implemented as separate Pro and Standard/Home products, passes every local gate, and has final amended-diff Opus approval for commit.
...
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 788 game records plus one explicitly classified non-game diagnostic after reviewed physical/virtual product splits. Current local author-ready game coverage is 34/788 (4.3147%), with 70 partial games and 684 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. The former Batman clone-root stub is now two physical records: the 2008 Pro/commercial product covers `bdk_130` through `bdk_294`, while `bdk_300` alone resolves to the materially different 2010 Standard/Home/Costco product. Stern 24, both Iron Man products, Avatar Pro/LE, TRON: Legacy Pro/LE, Rolling Stones Standard/LE, Transformers, X-Men, Star Trek, Mustang, Walking Dead, Avengers, Metallica, all five AC/DC products, Spider-Man 2007/Vault, Ali seven-digit conversion, and Cheech & Chong remain author-ready. |
| Semantic evidence import | In progress | Existing C#/JSON data, 55 PinMAME simulations, and 1,500 scripts from the two pinned VPX corpora have been extracted with provenance. A known-working VPX script has encoded precedence for runtime addresses, callbacks, ball routing, initial state, and mechanism causality; manuals remain authoritative for physical construction and PinMAME for emulator metadata. The isolated LibPinMAME harness captures segment text plus raw DMD pixel hashes/optional grayscale artifacts and snapshots solenoids from callback state rather than the legacy SAM query alias; it now optionally snapshots DMD/output state while a diagnostic switch remains held. Batman combines the organized native-text 207-page Stern manual, the exact working `bdk_294` standalone script, IPDB 5307/5583 product review, exact Pro/Home ROM hashes, a corrected four-ball Pro boot and three-ball Home boot, a complete held V3.00 switch sweep, and exact Home lamp/coil selector runs. This resolves both complete 1-64 matrices, dedicated/DIP inputs, Q1-Q32, model-specific game-on behavior, unused 51-66 capacity, lamps 1-88, GI 0, DMD, ball paths, Joker lock/drop/reveal, Batmobile bridge/fixed-ramp differences, upper mini-playfield omission, and both crane address maps. The Home diagnostics also expose a pinned PinMAME metadata defect: real lamps 30/46 inherit Pro `NONE` output types and must be restored at the same addresses, while unused Home 47 must be ignored rather than remapped. |
| Recreation knowledge | In progress | A conspicuous stub note exists for every unresolved root. Complete source-linked notes now additionally cover both Batman physical products. Pro notes preserve the four-ball trough, separate Joker lock/drop/reveal, one-direction three-position Joker motor, six-position chain crane and its two factory assembly variants, moving Batmobile bridge/captured-ball/crash transaction, elevated mini-playfield, device parts, lamps, and exact startup state. Home notes preserve the readdressed three-ball path and crane, single Joker lock, fixed Batmobile ramp, missing Joker reveal/drop and mini-playfield, Q26 flasher identity, absent fast-flip 33 callback, and actionable lamps 30/46 output-type correction without projecting Pro hardware onto the product. Earlier detailed product notes remain unchanged. |
...
| Validation and Opus review | In progress | The first persistent-session Batman review withheld approval for one blocker, one major, and seven minors. The generator's negative Pro crane-position labels are corrected; Home skill-shot switch 26 is sustained; Home pulse/sustained values are explicitly documented as device-class/Pro-analogue inferences rather than ROM-sweep observations; duplicate lamp 44 is disambiguated without inventing a semantic; crane-board part provenance, cabinet-button mechanism exclusions, shooter opto type, machine-specific Q20 wire color, focused position/pulse fixtures, and Home D16 evidence notes are remediated. Opus then returned `APPROVED: safe to commit` with one non-blocking knowledge-note minor; the Home knowledge page now repeats both the contact-duration inference and D16's evidence boundary, with a focused fixture. Twelve focused Batman fixtures and all 208 regressions pass; repository validation, deterministic regeneration, and `git diff --check` are green. Opus inspected the final amendment and again returned `APPROVED: safe to commit`; no Batman review gate remains. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, DMD-capable reusable LibPinMAME service/gameplay harness, and first 34 local author-ready records are implemented. Work continues newest-to-oldest without awarding completion credit to stubs or partial records:
...
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, both Transformers editions, Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, both Iron Man physical products, and Stern 24 are committed. Batman: The Dark Knight Pro/Home is fully validated and approved for its per-game commit. After that commit, process every existing partial definition newest-to-oldest before touching a new stub, grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly.
