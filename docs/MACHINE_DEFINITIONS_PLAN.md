...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 785 physical-game records plus one explicitly classified non-game diagnostic. Spatial migration deliberately changes the 33 current physical author-ready records to schema-v2 partials with `spatial_placement` missing and unknown, yielding 0/785 completion credit, 102 partial games, and 683 conspicuous game stubs; Scott's diagnostic is separately partial and excluded from the denominator. Existing semantic data and knowledge completeness are retained. |
