...
Status: Implementation in progress (2026-08-03, 54%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, both Iron Man physical products, Stern 24, and Batman: The Dark Knight received the same explicit approval. Batman is committed. Ripley's Believe It or Not! is the newest record promoted from partial: its definition, reusable Whitestar profile, runtime evidence, structured startup state, and recreation knowledge are undergoing first-review fixes before the full local gate and persistent Opus re-review required for its per-game commit.
...
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 788 game records plus one explicitly classified non-game diagnostic after reviewed physical/virtual product splits. Current local author-ready game coverage is provisionally 35/788 (4.4416%), with 69 partial games and 684 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. Ripley's now covers all 25 `ripleys`/`rip300`/`rip301`/`rip302`/`rip310` language and firmware drivers as one physical product, but receives final workflow credit only after Opus approval and commit. Earlier author-ready products remain unchanged. |
...
| Validation and Opus review | In progress | Ripley's has eight focused fixtures covering identity, all address spaces, pulse semantics, Q/public-output reconciliation, lighting/displays, custom mechanisms, reference uniqueness, exact evidence hashes, and manual-derived service hardware. Luna xhigh reported no mechanical-integrity findings; Terra xhigh independently reconciled Whitestar Q/public mappings, optional AUX limits, display topology, and harness evidence limits. Focused tests and repository validation pass. The remaining gates are deterministic regeneration, all regressions, diff hygiene, and explicit approval from the persistent Opus session before commit. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers, eight omitted abstract parent containers, and 773 clone-tree roots currently resolved to 788 game records plus one non-game diagnostic after reviewed product splits, including separate Iron Man original/Vault, Avatar Pro/LE, and TRON Pro/LE records. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, DMD-capable reusable LibPinMAME service/gameplay harness, and first 35 provisional local author-ready records are implemented. Work continues newest-to-oldest without awarding completion credit to stubs or partial records:
...
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, both Transformers editions, Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, both Iron Man physical products, Stern 24, and Batman: The Dark Knight Pro/Home are committed. Finish Ripley's validation and Opus review, commit it alone, then continue through the remaining partial definitions newest-to-oldest before touching a new stub, grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly.
