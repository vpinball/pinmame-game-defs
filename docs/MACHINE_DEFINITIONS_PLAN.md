...
Status: Implementation in progress (2026-08-03, 52%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, and both Iron Man physical products received the same explicit approval. Stern 24 is implemented and locally validated as one physical product spanning all four firmware drivers. Its first persistent-session Opus pass withheld approval over three majors; the amended batch removes unverified suitcase phase ordering, distinguishes switch semantics from unknown factory contact construction, splits every diagnostic artifact, and documents the ROM-proven high-side power wiring independently of Q control banks. Opus then returned `APPROVED: safe to commit`; after its four remaining cleanup minors were remediated, the final amended-diff confirmation again returned `APPROVED: safe to commit`.
...
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 787 game records plus one explicitly classified non-game diagnostic after reviewed physical/virtual product splits. Current author-ready game coverage is 32/787 (4.0661%), with 70 partial games and 685 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. Stern 24 now exhaustively groups `twenty4_130`, `twenty4_140`, `twenty4_144`, and `twenty4_150` under the February 2009 physical product; the final driver's 2010 catalog year is firmware chronology, not a hardware edition. Both Iron Man products, Avatar Pro/LE, TRON: Legacy Pro/LE, Rolling Stones Standard/LE, Transformers, X-Men, Star Trek, Mustang, Walking Dead, Avengers, Metallica, all five AC/DC products, Spider-Man 2007/Vault, Ali seven-digit conversion, and Cheech & Chong remain author-ready. Batman: The Dark Knight is the next newest unresolved record. |
| Semantic evidence import | In progress | Existing C#/JSON data, 55 PinMAME simulations, and 1,500 scripts from the two pinned VPX corpora have been extracted with provenance. A known-working VPX script has encoded precedence for runtime addresses, callbacks, ball routing, initial state, and mechanism causality; manuals remain authoritative for physical construction and PinMAME for emulator metadata. The isolated LibPinMAME harness captures segment text plus raw DMD pixel hashes/optional grayscale artifacts and snapshots solenoids from callback state rather than the legacy SAM query alias; it now optionally snapshots DMD/output state while a diagnostic switch remains held. Stern 24 combines its organized official partial 72-page Pink/Blue book, a pinned exact-ROM working script, IPDB 5419, Stern's product page, authenticated VPU release history, exact V1.3-V1.5 ROM archives/readme, boot/start evidence, two separately hashed held ROM Switch Tests, and separately hashed coil/lamp/Suitcase-menu diagnostics. This resolves every matrix/dedicated/DIP input, Q1-Q32, game-on 33, explicit unused SAM auxiliary capacity 51-66, lamp 1-80, GI 0, DMD, four-ball path, four-address suitcase stepper drive and three-ball lock, Safe House and Sniper facades, drops, gates, and posts. Standard SAM connector pins are sourced narrowly from a later official manual using the same boards and never used as 24 semantic evidence; the ROM Coil Test supplies machine-specific high-side wire colors independently of low-side Q connector banks. |
| Recreation knowledge | In progress | A conspicuous stub note exists for every unresolved root. Complete source-linked notes now additionally cover Stern 24. They preserve its four-ball trough/shooter state, physical Tournament Start at switch 15 despite a VPX bookkeeping pulse, 24 V suitcase drive positions Q21/Q23/Q25/Q30 without inventing A/B/C/D order, Q21-only VPX animation callback, home sensor, visible-lock ordering, release post, balls locked with the suitcase open or closed, exploding Safe House facade and saucer, opening and firing Sniper House with up-post, independently resetting single drops, control gates, unsensed ramp post, route contact duration, and the distinction between real hardware, optional Q8/Q24, synthetic game-on, and unused emulator capacity. Exact factory contact construction remains explicitly unknown where the partial book does not support it, while functional binding and pulse/sustained causality are validated. Earlier detailed product notes remain unchanged. |
...
| Validation and Opus review | In progress | Repository validation passes, all 196 regression tests pass, `git diff --check` is clean, and repeated curator/catalog/report generation produces an identical diff hash. Ten focused Stern 24 fixtures lock the physical identity and driver family, complete input/output/lamp spaces, sustained contacts, Tournament Start correction, suitcase drive addresses and evidence limits, machine-specific high-side wiring, custom-mechanism causality, narrow source boundaries, exact evidence hashes, and unique bindings/actuator ownership. Fifteen harness fixtures additionally cover the held-snapshot helper label/index contract. The first Opus pass withheld approval over three majors and five minors; after remediation it returned `APPROVED: safe to commit` with no blocker or major. All four residual cleanup minors were fixed, and its final amended-diff confirmation again returned `APPROVED: safe to commit`. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers, eight omitted abstract parent containers, and 773 clone-tree roots currently resolved to 787 game records plus one non-game diagnostic after reviewed product splits, including separate Iron Man original/Vault, Avatar Pro/LE, and TRON Pro/LE records. |
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, DMD-capable reusable LibPinMAME service/gameplay harness, and first 32 author-ready records are implemented. Work continues newest-to-oldest without awarding completion credit to stubs or partial records:
...
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, both Transformers editions, Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, both Iron Man physical products, and Stern 24 are complete locally. Obtain Opus approval and commit the Stern 24 batch, then process Batman: The Dark Knight and continue backward while grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly before curation.
