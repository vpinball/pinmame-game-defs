...
| Static catalog site | Ready after T2 integration | Copy the existing external `pinmame-games` source into `site/` without build caches or generated output, install its Pages workflow at repository root, configure the custom-domain build for `games.visualpinball.org`, build and verify it against the combined catalog, obtain separate Opus approval, commit, push the fast-forward integration branch to the repository's default `master` branch, verify deployment, and report the exact DNS target. |
...
1. Finish Terminator 2's combined integration review and commit, remove its clean game worktree, then import, review, publish, and verify the static site. Preserve the existing X-Men Pro, Transformers Pro, TRON Pro, Avatar Pro, and Iron Man 2010 worktrees without beginning further Pro searches, then resume the remaining non-Pro partials newest-first.
...
5. Finish the user-edited continuation runbook through its separate Opus and integration gates; copy and verify the static catalog site without transient build artifacts, obtain separate Opus approval, configure the custom domain, fast-forward the combined branch to the remote default branch, and verify GitHub Pages before reporting the DNS target.
