...
Status: Implementation in progress (2026-08-02, 48%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; the corrected Rolling Stones Standard/LE delta has now received the same explicit pre-commit approval.
...
| VPE-neutral schema and toolchain | In progress | Machine, catalog, controller, evidence, manual-cache, and manual-extraction schemas are implemented with canonical hashing, exact registry rebuilding, fail-closed validation, domain-specific evidence precedence, controller profiles, and reusable runtime-observation fields for switch-to-lamp causality, GI, initial lamp state, and named outputs. Repository validation now reconciles every runtime-observed solenoid/lamp/GI address against author-ready definitions, enforces the global virtual SAM game-on state at public solenoid 33, keeps optional physical ticket-service identities in an explicitly untransported group, rejects duplicate physical output connections and mechanism actuator ownership, and validates manual manifests/extractions before use; canonical merge/canonicalization and full fixtures remain. |
| Author-ready definitions | In progress | The pinned public API produced 2,873 driver records grouped into 784 game records plus one explicitly classified non-game diagnostic record after reviewed physical-product/virtual-product splits, the Rolling Stones, Transformers, X-Men, and other edition splits, and the Ali conversion-family merge. Current author-ready game coverage is 25/784 (3.1888%), with 72 partial games and 687 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the game denominator. The 25-driver AC/DC family is split into complete Premium/LE/LUCI, original Pro, 2014 LED Pro, Vault Edition, and American Country contracts. The formerly merged Metallica family is now split into complete Premium/LE and Pro contracts with their incompatible hardware kept separate. Rolling Stones Standard and Limited Edition now exhaustively split all five `rsn_` drivers, with the four-ball Standard and five-ball ceramic/magnet/post LE hardware kept distinct. Both physical Transformers, X-Men, Star Trek, Mustang, Walking Dead, and Avengers products are separate author-ready records, alongside Spider-Man 2007, Spider-Man Vault Edition, the Ali seven-digit conversion, and Cheech & Chong. |
| Semantic evidence import | In progress | Existing C#/JSON data, 55 PinMAME simulations, and 1,500 scripts from the two pinned VPX corpora have been extracted with provenance. A known-working VPX script has encoded precedence for runtime addresses, callbacks, ball routing, initial state, and mechanism causality; manuals remain authoritative for physical construction and PinMAME for emulator metadata. A reusable isolated LibPinMAME harness now captures switch injection, segment text frames, solenoid transitions, public WPC lamp-matrix addresses, lamp/GI changes, initial switch state, snapshots, exact ROM/run hashes, and schema-validated runtime evidence, including reusable display-layout and observed-solenoid fields. Rolling Stones combines an organized official 99-page Stern scan, the exact proven LE script, Stern/IPDB physical identity, pinned `SAM_NO_AUX` behavior, and exact `rsn_110`/`rsn_110h` boot-start traces. This resolves the complete shared 64-switch/80-lamp/32-output chart, Moving Mick's seven sensed positions plus dedicated hit input, center lock/latch, Standard omissions, LE ceramic-ball detector, two magnets, three up/down posts, real D13/D15 cabinet buttons, and virtual-only D14/D16 VPX compatibility states. Transformers combines byte-identical organized official Pro/LE manual downloads, the exact proven Pro script, contemporary physical-machine review, pinned SAM auxiliary serialization, and exact `tf_180`/`tf_180h` boot-start traces. This resolves the Pro's script-only output-4 gate callback, the LE Q41-Q46 to public 59-64 translation, distinct lamp maps, four-ball Megatron trough, Optimus ramp/toy, LE drop target/cannon, Starscream endpoints, and Ironhide mini-playfield. Exact AC/DC Pro, Vault, and LUCI scripts plus official Pro, Premium, and LUCI manuals resolve the original/Premium/LED/Vault product differences, public auxiliary-coil remapping, BGR extended lamps, passive versus magnetized bells, and cannon causality; exact acd_170 and acd_170h boot/start runs anchor the DMD, GI, and public-output observations. Metallica Premium/LE combines the official 183-page manual, exact VPW script, exact local VPX object addresses, pinned PinMAME RGB/GI and magnet-board routing, and an exact mtl_180h boot/start run to validate all public I/O, six RGB connector triples, four GI colors, and the coffin processor's two-bit behavior. Metallica Pro now combines the same manual's Pro addendum, JP's authenticated VPForums 6.0.0 table and extracted `mtl_180` script, and a family-compatible `mtl_170` harness run to validate its ordinary 1-80 lamp matrix, GI 0, outputs 1-35, four-ball initialization, switch polarity, and absence of Premium-only hardware. Star Trek Pro combines the official 173-page scan, an exact archived working VPX/script, physical lamp-object inspection, pinned SAM public-address rules, and an exact `st_161c` boot/start trace to validate its ordinary 1-80 lamp matrix, GI 0, outputs 1-32, four-ball initialization, passive Vengeance/kickback, model-specific target/ramp topology, and absence of LE-only node and auxiliary boards. X-Men Limited Edition combines organized official and high-resolution manual copies, the exact VPW script, pinned SAM auxiliary-board routing, Stern's model inventory, IPDB identity, and an exact `xmn_151h` boot/start trace; this validates its 80-lamp matrix, Q41-Q48/public 51-58 mapping, subtractive color GI, Iceman ramp, two mechanically latched Nightcrawlers, independent Magneto disc/magnet, Wolverine magnet, and four-ball lock. X-Men Pro combines the same organized manual's common service topology with an authenticated VPU table, its extracted `xmen_150` script, full object inventory, coordinates, and hash-pinned playfield art; this validates all 80 ordinary lamp channels, GI 0, 32 main outputs, every flasher group, two magnets, four-level lock, missing LE hardware, and the physical output-12/input-86 upper flipper despite the legacy table's output-16 visual shortcut. Exact model-specific Mustang, Avengers, and Walking Dead sources perform the same role for those editions; Mustang Pro additionally proves the non-linear Board-5 mapping between printed diagnostic lamps 98-108 and public channels 102-112. |
| Recreation knowledge | In progress | A conspicuous stub note exists for every candidate root. Complete source-linked notes now cover all five AC/DC products, both Metallica editions, both physical Spider-Man, Star Trek, Mustang, Walking Dead, Avengers, X-Men, Transformers, and Rolling Stones products, the Ali seven-digit conversion, and Cheech & Chong. The Rolling Stones notes preserve the typed trough and shooter state, center lock/latch, Moving Mick's seven position windows and shared target-hit switch, physical route/target inventory, complete lamps/flashers, and exact Standard/LE exclusions; the LE note additionally captures ceramic-ball identity through drain cycles, detector polarity sequence, magnetic response, all three raised-post collisions, real post buttons, and nonphysical VPX compatibility inputs. The Transformers notes preserve the common trough, Megatron stack, Allspark eject, Optimus ramp/toy, Bumblebee captive-ball area, complete routes, targets, lamps, and explicit Pro/LE boundary; the LE note additionally captures the drop target, animated Megatron/cannon, sensed Starscream platform, four-opto player-tilted Ironhide mini-playfield, two gates, and auxiliary address translation. X-Men LE preserves four-ball startup, the four-level Magneto lock, independent disc and PWM magnet, two sensorless-upper-position Nightcrawler latches, the two-endpoint ball-carrying Iceman ramp, Wolverine bash/magnet behavior, eject routes, flipper gating, target inventory, and subtractive color-GI polarity. X-Men Pro instead preserves its ordinary 58-lamp playfield with exact coordinates, static Nightcrawler-themed inserts, two magnets, passive Wolverine bash toy, four-level lock, output-specific flasher layout, two ramps/orbit network, absence of every LE-only mechanism, and the resolved upper-flipper shortcut. The Star Trek Pro note preserves the four-ball state, center memory target and magnet lock, passive Vengeance bash/kickback, left eject and auto-launch constants, laser, all target banks and sensed ball paths, six RGB insert channel triples, exact Pro/LE exclusions, and the resolved legacy-table upper-flipper shortcut. The Metallica Premium/LE note preserves the four-ball initial state, corrected shooter and toy opto addresses, motorized grave marker, electric-chair magnet and step-up, hammer/captive-ball causality, three-bank reset, loop post, coffin lock, lowering coffin magnet, four-state two-bit magnet processor, snake jaw/latch/eject behavior, exact RGB connector channels, and color GI contract. The Pro note instead preserves its static grave-marker magnet and inline drop bank, passive captive ball, static snake eject, unsensed loop post, ordinary GI/lamps, optional cabinet hardware, and explicit prohibition on importing the Premium coffin, hammer, moving marker, motorized jaw, RGB/GI-board, or decorative VPX spinner objects as scoring devices. The AC/DC notes preserve the four-ball startup state, cannon position/sensors, ball routes, drop banks, lower mini-playfield, auxiliary-board address translation, RGB/GI topology, passive versus magnetized bell behavior, American Country retheme boundary, and the proven VPX/manual conflict at lamps 14, 15, and 17. The Mustang Pro note preserves its script-compatible six-ball trough ordering, two hold-driven ramp routes, paired orbit posts, scoop transfer/eject behavior, captive-ball constants, passive bowl, and Board-5 public/manual address shuffle. |
...
| Validation and Opus review | In progress | Repository validation passes with exact definition/catalog driver membership and stronger author-ready gates; 147 evidence-policy, manual-cache/extraction, ROM-index, classification, runtime-harness, AC/DC, Metallica, Avengers, Ali, Cheech & Chong, Walking Dead, Mustang, Star Trek, X-Men, Transformers, and Rolling Stones regression tests pass. Scanned-manual detection ignores whitespace and page-number-only text layers; game/non-game accounting, edition separation, physical-model identity, auxiliary-board remapping, exact source hashes, controller-plugin group IDs, WPC public address ranges, trough topology, service-output translations, discrete SCR address holes, custom mechanisms, actuator ownership, physical connector uniqueness, runtime-observation reconciliation, SAM game-on semantics, manual-download integrity, target/rollover mappings, extended-lamp mappings, complete AC/DC, Metallica, Star Trek, X-Men, Transformers, and Rolling Stones address spaces, product-specific lamp/GI/mechanism deltas, inherited virtual-retheme contracts, and ROM/script activity that must not invent physical outputs are protected by fixtures. The persistent Opus session approved the committed foundation and then completed two Rolling Stones review passes plus a narrow provenance verification, explicitly approving the corrected delta as safe to commit. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers, eight omitted abstract parent containers, and 773 clone-tree roots currently resolved to 784 game records plus one non-game diagnostic after reviewed Rolling Stones, Transformers, X-Men, Spider-Man, The Walking Dead, Mustang, Star Trek, Avengers, AC/DC, and Metallica product splits and the Ali conversion-family merge. |
...
1. `catalog/index.json` is generated from PinMAME and the curated machine registry. It maps every PinMAME driver ID to a physical machine, definition file, definition version, compatibility status, and content hash.
...
Project completion is measured over unique physical machines, not ROM count: every supported PinMAME driver must resolve to a physical machine, and every such machine must resolve to an `author_ready` definition. ROM clones may inherit a shared complete definition when hardware and playfield I/O are identical; actual variant differences require explicit overrides. Generated stubs prove catalog reachability only and contribute zero to author-ready coverage.
...
Build `tools/pinmame-catalog` to load or link a specific LibPinMAME build, call `PinmameGetGames`, and emit every driver with description, year, manufacturer, clone parent, flags, and generator revisions. `PinmameGame.found` depends on the local ROM path, so exclude it from the canonical hashed catalog and emit it only in an optional local availability report generated with an explicit ROM-path policy. Generate candidate physical-machine families and an exception report; never silently change curated family assignments.

Acceptance gate: every driver returned by the pinned build appears exactly once in the catalog; every clone parent resolves; generated output is byte-for-byte reproducible; and a catalog diff clearly separates added/removed drivers, metadata changes, and family-assignment changes.
...
Exit gate: every driver returned by the pinned build resolves to exactly one catalog record and candidate physical machine; every clone parent resolves; the generated manifest records the actual count; selected cross-platform samples enumerate correctly; no generic generated label is presented as a validated semantic name; and every structurally generated record is visibly classified as a stub until the author-readiness validator proves otherwise.
...
The RFC/validator vertical slice, catalog generator, legacy import, two-corpus VPX extraction, organized manual acquisition/extraction, ROM corpus indexer, reusable LibPinMAME service/gameplay harness, and first 21 author-ready records are implemented. Work continues newest-to-oldest without awarding completion credit to stubs or partial records:

1. Record the Mustang Ghidra pilot as inconclusive unless its incomplete headless log can be made reproducible; retain static analysis only when exact-ROM results add semantic evidence beyond the manual and working script.
2. AC/DC LUCI Premium, both Metallica editions, both Star Trek editions, both X-Men editions, both Transformers editions, and Rolling Stones Standard/LE are complete. Process the existing Tron Legacy LE partial plus its Pro sibling next, then continue backward while grouping clone, color-ROM, conversion, virtual, and product-edition variants explicitly before curation.
3. Prefer a known-working VPX script for controller-facing runtime semantics, official/manual evidence for physical construction and wiring, and pinned PinMAME source for transport topology; acquire manuals into the reusable external cache and record exact hashes and page locators.
4. For each machine, write recreation knowledge for custom mechanisms, ball paths, startup state, timing, geometry clues, and authoring pitfalls; keep the definition partial whenever any authoring-critical behavior remains unvalidated.
5. Extend deterministic harness scenarios where scripts/manuals leave gaps, using the authorized external ROM corpus and storing only hashes/traces. Use targeted Ghidra analysis only after cheaper sources fail.
6. Keep schema validation, exact catalog reachability, coverage reporting, and regression tests green after every curation batch. The first checkpoint completed the required persistent Opus review, two remediation rounds, final approval, and green validation before commit; resume the same session for later high-risk reviews.
