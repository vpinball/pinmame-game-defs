...
Status: Implementation in progress (2026-08-03, 55%), based on the Opus-reviewed architecture and the discussion in [pinmame-game-defs PR #1](https://github.com/vpinball/pinmame-game-defs/pull/1). The complete first implementation checkpoint and its review fixes received final approval from the persistent Opus session before commit; Rolling Stones Standard/LE, TRON: Legacy Pro/LE, Avatar Pro/LE, both Iron Man physical products, Stern 24, Batman: The Dark Knight, and Ripley's Believe It or Not! received the same explicit approval. Ripley's is committed at `3d10c8a`. The current cleanup enforces the user-confirmed physical-machine-only scope before Terminator 2 becomes the next game.
...
| Author-ready definitions | In progress | The pinned public API returns 2,873 drivers; seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers grouped into 786 physical-game records plus one explicitly classified non-game diagnostic. Current local author-ready physical-game coverage is 33/786 (4.1985%), with 69 partial games and 684 conspicuous game stubs receiving no completion credit; Scott's diagnostic is separately partial and excluded from the denominator. Ripley's covers all 25 `ripleys`/`rip300`/`rip301`/`rip302`/`rip310` language and firmware drivers as one physical product and is committed. |
...
| Validation and Opus review | In progress | Ripley's has eight focused fixtures covering identity, all address spaces, pulse semantics, Q/public-output reconciliation, lighting/displays, custom mechanisms, reference uniqueness, exact evidence hashes, and manual-derived service hardware. Its 216-test gate, repository validation, deterministic regeneration, diff hygiene, and persistent Opus re-review passed before commit. The physical-only scope cleanup is now under validation and requires its own explicit Opus approval before commit. |

### Pinned implementation inputs

| Input | Revision | Current inventory |
| --- | --- | --- |
| `vpinball/pinmame` | `4ec52ff0ac133ac251681518aed2249e19fe26eb` | Fresh Windows x64 LibPinMAME build; public API returns 2,873 drivers. Seven custom-ROM-only virtual-table drivers are excluded, leaving 2,866 in-scope drivers, eight omitted abstract parent containers, and 772 clone-tree roots resolved to 786 physical-game records plus one non-game diagnostic. |
