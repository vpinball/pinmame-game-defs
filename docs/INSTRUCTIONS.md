...
- Every one of the 779 physical-game records is `author_ready`, every supported physical/controller variant is accounted for, and the generated completion gate is true. Stubs contribute zero coverage; partials are not publishable as complete entries; the non-game records (the retained diagnostic plus the ten test-fixture/test-chip drivers classified `diagnostic_software`) remain separately classified.
