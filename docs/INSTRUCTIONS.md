...
- On System 3 through System 11 and Data East derivatives, keep a manual's printed special-solenoid number, the PIA handler index, `sxx.ssSw` slot, and PinMAME public output address distinct. Derive the public address from the selected `setSSSol` permutation when the PIA drives the circuit, and from the direct `CORE_FIRSTSSSOL + ii` path when `sxx.ssSw[ii]` supplies switch-driven state; Data East's PIA permutation is not sequential. Preserve the printed number as an alias and test every public binding. Do not assign printed SP1-SP6 mechanically to addresses 17-22 unless primary game-specific evidence explicitly pairs each public address and SP identity; if that evidence conflicts with the shared-source derivation, use the explicit game table as the working mapping, mark every affected binding conflicted, name the sibling definitions that use the source-derived contract, and require a LibPinMAME trace before resolution. Secret Service is the current disputed example; Playboy, Time Machine, and Torpedo Alley retain the source-derived permutation because their evidence does not provide the same explicit public-number pairing.
