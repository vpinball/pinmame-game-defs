#!/usr/bin/env python3
"""Deterministic curator for Bally Kiss (1979).

Regenerates ``machines/partial/bally/kiss-1979.json`` and its knowledge note from the
...
The tables are the curated result of the evidence sources in the priority the runbook sets
out: the Bally game #1152-E manual for physical construction and lamp connector functions,
the ROM's own solenoid self test (captured by the LibPinMAME harness) for the
printed-to-public solenoid mapping, the AS-2518-43 board schematic for the auxiliary lamp
wiring, and pinned PinMAME for controller topology. The self-test mapping in particular must
not be "tidied" into an identity mapping - see ``knowledge/bally/kiss-1979.md``.
...
PARTIAL_PATH = ROOT / "machines" / "partial" / "bally" / "kiss-1979.json"
AUTHOR_READY_PATH = ROOT / "machines" / "author-ready" / "bally" / "kiss-1979.json"
...
def build_definition() -> dict:
	...
	for key, spec in sorted(DATA.get("dips", {}).items(), key=lambda item: int(item[0])):
		...
		inputs.append({
			...
			"provenance": spec["provenance"],
			"spatial": {
				"provenance": {"source_refs": ["manual.bally.kiss.1979"], "status": "validated"},
				"reason": "dip_switch",
...
def main() -> int:
	...
	if args.check:
		...
		if drifted:
			return 1
		print("Kiss definition and knowledge note match the deterministic curator.")
		return 0
