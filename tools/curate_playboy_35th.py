...
def conflicts() -> list[dict[str,object]]:
    return [
        ...
        {"id":"conflict.muxed-c-bank-core-bulb-type-vs-manual-coils","path":"/outputs/coil.driver-25","description":"Pinned Playboy s11.c uniformly assigns #89-bulb PWM type to public 25-32. The rendered manual fits Outhole, Trough, VUK, Champagne Kicker, Drop Reset, Grotto Kicking and Knocker coils at 25-30/32 and no R07 branch at 31. Core numbering/timing and manual construction are retained without treating the type metadata as fitment authority. Resolution path: a corrected play_ output-typing block in upstream src/wpc/s11.c, which a PinMAME contributor can submit because the printed schematic already itemizes real coils by part type at 25-30 and 32; separately, whether public 31 carries any meaningful state needs a LibPinMAME gameplay trace, because the retained play_a24 automatic Coil Test cycle never asserted that address and records that absence as a stated limitation rather than as proof it is dead. Unresolved.","source_refs":[CORE_SOURCE,MANUAL_SOURCE,SCRIPT_SOURCE]},
        {"id":"conflict.drop-reset-coil-type-print","path":"/outputs/coil.driver-29","description":"The rendered output schematic appears to print Drop Target Reset as 23-1200, while the adjacent coil part-number chart prints one 12-1200 coil and no 23-1200. No correction is inferred. Resolution path: a Data East parts catalogue entry for assembly 090-5008-00, the one assembly the same chart pairs with its single 12-1200 coil, or a photograph of the coil sleeve on an unrestored Drop Target Reset assembly showing which type is stamped on it. Unresolved.","source_refs":[MANUAL_SOURCE,RENDER_SOURCE]},
        {"id":"conflict.special-solenoid-19-vs-background-proxy","path":"/outputs/coil.driver-19","description":"Pinned Data East setSSSol maps the manual's SP4 left-pop-bumper circuit to public 19. The active table additionally maps public 19 to pseudo-lamp 119 and off-playfield FL19 as a background flasher. The proxy is recorded as a consumer side effect, not a physical coil location. Resolution path: a LibPinMAME gameplay-harness trace against a legal play_a24 ROM that starts a ball and closes the left pop-bumper switch while watching public 19; the retained play_a24 diagnostic run cannot show it, because this driver's ssSw mapping is empty and the ROM's automatic Coil Test never reaches 17-22. Unresolved.","source_refs":[MANUAL_SOURCE,CORE_SOURCE,SCRIPT_SOURCE,TABLE_SOURCE]},
        {"id":"conflict.flash-location-drawing-vs-playfield-proxies","path":"/outputs/coil.driver-3","description":"The manual's separate backbox inset clearly assigns bulbs to outputs 3, 4, 8, 9, 12, 14 and 15, while every retained VPX effect for those outputs is located in playfield space. Output 12 alone has four clear backbox symbols, equal to its complete schematic quantity, yet three playfield effect objects. Small 5/6-like and 13/16-like inset glyphs remain unassigned. Each output note preserves the certain minimum; VPX effects are not promoted into extra physical sockets. Resolution path: a higher-resolution scan of printed page 26's backbox inset from a second copy of this manual, resolving the small unassigned 5/6-like and 13/16-like glyphs, or a photograph or socket survey of an unrestored backbox counting the flash bulbs actually wired to each of outputs 3, 4, 8, 9, 12, 14 and 15. Unresolved.","source_refs":[MANUAL_SOURCE,TABLE_SOURCE,SCRIPT_SOURCE]},
    ]
...
def build_spatial(machine: dict[str,object]) -> dict[str,object]:
    ...
    # An ignored conflict is not outstanding work, so it is not a blocker.
    unresolved_conflict_ids = [conflict["id"] for conflict in unresolved_conflicts(machine)]
    return {
        ...
        "promotion_decision":"Keep partial. The record enumerates every public address and resolves the major topology, but output annotations/typing, physical polarity, special-coil behavior, lamp socket placement, and eight unresolved conflicts prevent author-ready promotion.",
    }
