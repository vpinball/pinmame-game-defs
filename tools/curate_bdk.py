...
EXCERPT_ROOT = REPOSITORY_ROOT / "evidence/excerpts/stern.batman-the-dark-knight.2008"
...
HOME_SPATIAL_GAP_SOURCE = "review.batman-the-dark-knight-standard-home.spatial-source-gap"
...
def batman_manual_source() -> dict[str, object]:
	base: dict[str, object] = {"id": MANUAL_SOURCE, "kind": "manual", "uri": "https://www.sternpinball.com/wp-content/uploads/2018/11/Batman_TDK_Manual.pdf", "sha256": "c09621893c439a966d2c48436b482fdf6326bc99ed5acd45a5634d5ae39c3219", "locator": "Official 207-page operation and parts manual. PDF pages 6, 8, and 10 are the complete Pro switch, lamp, and Q1-Q32 grids; pages 85-96 document the two Scarecrow crane assembly variants; the game-specific diagnostics describe Joker and crane motor tests. For the Home definition it is used only for the shared SAM connector topology and retained crane-board context, never for Home semantic addresses or omitted Pro mechanisms.", "license": "NOASSERTION", "attribution": "Stern Pinball, Inc.", "source_id": "stern", "original_filename": "Batman_TDK_Manual.pdf", "rights": "NOASSERTION", "acquired_at": "2026-08-03T05:50:06.472838Z"}
	entries = [
		("pro-switch-grid", "excerpt.batman.pro-switch-grid", "PDF page 6, switch matrix and playfield switch locations", "Batman_TDK_Manual.pdf page 6, crop box 0.03,0.08,0.97,0.92, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x632 WebP quality 45", "The page lays out matrix switches 1-48 and their physical labels, including the left and right trough contacts, shooter lane, lanes, outlanes, return lanes, slingshots, pop bumpers, Joker positions, Batmobile crash, and Scarecrow VUK."),
		("pro-dedicated-switches", "excerpt.batman.pro-dedicated-switches", "PDF page 8, dedicated switch and cabinet input table", "Batman_TDK_Manual.pdf page 8, crop box 0.03,0.04,0.97,0.92, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x662 WebP quality 45", "The continuation lists dedicated D-12 through D-16 inputs, flipper end-of-stroke and cabinet service contacts, and the crane contact; it also states that no switches are located on the back panel."),
		("pro-lamp-grid", "excerpt.batman.pro-lamp-grid", "PDF page 10, lamp matrix grid [#1-#80]", "Batman_TDK_Manual.pdf page 10, crop box 0.03,0.04,0.97,0.92, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x662 WebP quality 45", "The grid maps lamp rows 01-04 to named playfield lamps and wiring, including Joker, Start Button, Batmobile, Tournament, Scarecrow, shooter, lanes, Bruce Wayne, Commissioner Gordon, Rachel, Harvey Dent, Alfred, Batman and crane-position indicators."),
		("pro-coil-chart-q1-q8", "excerpt.batman.pro-coil-chart-q1-q8", "PDF page 13, coil and flasher detailed chart Q1-Q8", "Batman_TDK_Manual.pdf page 13, crop box 0.03,0.26,0.97,0.98, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x542 WebP quality 45", "The chart identifies Q1 trough up-kicker, Q2 auto launch, Q3 top-right eject, Q4 Joker lockup, Q5 Joker drop-target up, Q6 Scarecrow VUK, Q7 Joker drop-target down, and Q8 optional shaker motor."),
		("pro-coil-chart-q9-q19", "excerpt.batman.pro-coil-chart-q9-q19", "PDF page 14, coil and flasher detailed chart Q9-Q19", "Batman_TDK_Manual.pdf page 14, crop box 0.03,0.16,0.97,0.98, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x617 WebP quality 45", "The chart identifies Q9-Q11 as the pop bumpers; Q12 as the left control gate; Q13 and Q14 as the Batmobile ramp and control gate; Q15 and Q16 as the flippers; Q17 and Q18 as the slingshots; and Q19 as the Scarecrow home flasher."),
		("pro-coil-chart-q20-q31", "excerpt.batman.pro-coil-chart-q20-q31", "PDF page 15, coil and flasher detailed chart Q20-Q31", "Batman_TDK_Manual.pdf page 15, crop box 0.03,0.04,0.97,0.98, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x707 WebP quality 45", "The chart identifies the back-panel, Joker and Scarecrow flashers, optional coin meter, pop-bumper flashers, Joker motor, Scarecrow motor relay, right slingshot flasher, Joker motor relay and Scarecrow motor across the Q20-Q31 range."),
		("scarecrow-motor-diagnostic", "excerpt.batman.scarecrow-motor-diagnostic", "PDF page 50, game-specific Scarecrow motor diagnostic", "Batman_TDK_Manual.pdf page 50, crop box 0.03,0.04,0.97,0.94, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x677 WebP quality 45", "The service menu's Scarecrow motor test operates Q31, relay Q28, and switches 56 through 61; the manual warns not to move the crane by hand and to turn off power before servicing."),
		("scarecrow-crane-type-1", "excerpt.batman.scarecrow-crane-type-1", "PDF page 163 (printed Section 4, Chapter 2, page 85), Type 1 Scarecrow crane assembly", "Batman_TDK_Manual.pdf page 163, crop box 0.03,0.05,0.97,0.94, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x670 WebP quality 45", "The manual distinguishes two crane assembly types. Type 1 has a round hub hole and tear-drop-shaped motor shaft; it says not to move or pull the crane by hand and gives an 80-85 in-lb motor-mount torque specification."),
		("scarecrow-crane-type-2", "excerpt.batman.scarecrow-crane-type-2", "PDF page 170 (printed Section 4, Chapter 2, page 91), Type 2 Scarecrow crane assembly", "Batman_TDK_Manual.pdf page 170, crop box 0.03,0.05,0.97,0.94, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x670 WebP quality 45", "The manual distinguishes Type 2 by its square hub hole and square-shaped motor shaft, contrasting it with the round-hub Type 1 assembly; the following pages break down the two types separately."),
		("scarecrow-crane-type-2-parts", "excerpt.batman.scarecrow-crane-type-2-parts", "PDF page 175 (printed Section 4, Chapter 2, page 96), Type 2 crane arm parts list", "Batman_TDK_Manual.pdf page 175, crop box 0.03,0.03,0.97,0.90, born-digital page rendered for legibility (smallest type in region 11.6pt, targeting 11px glyphs), rendered at 68 dpi, grayscale, 547x655 WebP quality 45", "The parts list identifies the square-hub crane arm assembly, crane pendulum assembly, modified one-inch carbon ball, pendulum spring and hub, and Scarecrow-arm sensor bracket."),
	]
	excerpts = []
	for stem, excerpt_id, locator, derivation, _summary in entries:
		markdown = EXCERPT_ROOT / f"{stem}.md"
		image = EXCERPT_ROOT / f"{stem}.webp"
		if not markdown.is_file() or not image.is_file():
			raise FileNotFoundError(f"Batman manual excerpt is missing: {markdown} or {image}")
		excerpts.append({"id": excerpt_id, "locator": locator, "path": str(markdown.relative_to(REPOSITORY_ROOT)).replace("\\", "/"), "sha256": hashlib.sha256(markdown.read_bytes()).hexdigest(), "image": str(image.relative_to(REPOSITORY_ROOT)).replace("\\", "/"), "image_sha256": hashlib.sha256(image.read_bytes()).hexdigest(), "image_derivation": derivation, "method": "manual", "transcribed_by": "curator, read from the rendered page", "reviewed": True})
...
def home_sources() -> list[dict[str, object]]:
	return shared_sources() + [
		...
		{"id": HOME_IPDB_SOURCE, "kind": "human_review", "uri": "https://www.ipdb.org/machine.cgi?id=5583", "locator": "IPDB 5583 identifies a real 2010 Stern Standard Model physical pinball machine with S.A.M. hardware, an approximately 48-unit production run, playfield/cabinet photographs, and fewer toys/different software than the 2008 Pro; photos corroborate the simplified fixed Batmobile ramp and retained Scarecrow crane without supplying controller addresses.", "license": "NOASSERTION", "attribution": "Internet Pinball Database", "acquired_at": "2026-08-03T04:00:00Z"},
		{"id": HOME_SPATIAL_GAP_SOURCE, "kind": "human_review", "uri": "external:pinmame-review-artifacts/stern/batman-the-dark-knight-standard-home-edition-2010/source-gap-report.md", "sha256": "777ff0c79e0fba946d6164320dfe0dfca0dc4881b1efb3f58d7d375f8207e6c2", "locator": "Ordered exact-edition VPX/manual search ledger dated 2026-08-04. The two local table locations contain only 2008 Pro/other Batman tables; no exact Home VPX was retained. Authenticated searches as freezy found only VPUniverse file ID 5043, the exact bdk_300 ROM package, and no matching VPForums download, not an exact Home VPX. Stern's manual index, IPDB 5583, and an exact-edition Internet Archive search yielded no Standard/Home manual. No additional download was necessary or acquired. This is a source-gap record, not spatial evidence.", "license": "NOASSERTION", "attribution": "Local source-reconciliation review"},
...
## Exact-edition VPX search and spatial blocker

The ordered exact-edition table/manual search is recorded in `external:pinmame-review-artifacts/stern/batman-the-dark-knight-standard-home-edition-2010/source-gap-report.md`. `L:\\Visual Pinball\\Tables` and `L:\\Visual Pinball\\Tables Archive` contain Batman Dark Knight tables labeled Stern 2008/`bdk_294`, not the Standard/Home product. Authenticated searches as `freezy` found only VPUniverse file ID `5043`, the exact `bdk_300` ROM package, and no matching VPForums download, not an exact Home VPX table. Stern's manual index, IPDB 5583, and an exact-edition Internet Archive search yielded no Standard/Home manual. No additional download was necessary or acquired. The retained `bdk_294` script therefore remains semantic/comparative evidence only; it cannot provide Home geometry. Do not promote this definition until an exact physical Home VPX or an equivalent exact-edition coordinate review is acquired.
