...
the three-recreation spatial consensus, and every reviewed label, wiring detail and
...
The spatial consensus embedded here treats the three retained recreations as three measurements
of one physical machine. Independence between them is unestablished, so a placement is validated
only when all three agree; disagreeing tables are recorded rather than averaged, and the printed
location diagrams break ties.
...
TABLE_SOURCE = {"vpw-1-6": T_VPW, "jpsalas-600": T_JPS, "hanibal-4k": T_HAN}
...
SOURCES = [
    ...
    next(s for s in partial["sources"] if s["id"] == LEGACY),
    {
        ...
        "locator": (
            ...
            "real and are deliberately not enumerated here; see coverage.missing and the spatial blockers report; "
    ...
    },
    {
        ...
        "locator": (
            ...
            "conversion with dedicated inputs at -3..0 and 81-88, the lamp range capped at 1-80, and the legacy "
    ...
    {
        "attribution": "Stern Pinball, Inc.; scan retained from IPDB", "id": MANUAL, "kind": "manual",
        "license": "NOASSERTION", "rights": "NOASSERTION",
        "locator": (
            "IPDB machine 4858, machine page https://www.ipdb.org/machine.cgi?id=4858. 184-page image-only scan of the "
            "English service manual; no text layer, so every table was read from pages rendered at 300 dpi. PDF page 6 "
            ...
            "locations with the typical switch, lamp and coil wiring schematics. The direct resource URL of the scan "
            "was not captured at acquisition time and is recorded as unknown rather than reconstructed."
        ),
        "original_filename": "Stern_2003_The_Lord_of_the_Rings_English_Manual_complete.pdf",
        "sha256": "cd5878802071b273ff6072a80ab915249d45b11586dd29250261443c0fd4aee2",
        "uri": "external:pinmame-manuals/by-machine/stern.lord-of-the-rings.2003/Stern_2003_The_Lord_of_the_Rings_English_Manual_complete.pdf",
    },
    {
        ...
        "revision": "2026-08-06",
        "sha256": "142543f85acac7045cf3f3ffd414d27bf7df4f5eb0e5d2f5fb6afbff8535bc73",
    ...
    },
    {
        ...
        "license": "NOASSERTION",
        "revision": "2026-08-06",
        "sha256": "c42675d22e56eab77fbe7f8b5c4fc81c9588e9c1d367d5b0fd0bcb4fa35bfe27",
        "locator": (
            "Three-table spatial consensus produced by tools/lotr_spatial_resolve.py from the retained extractions. "
            "It is the sole authority for every coordinate in this definition: for each device it records each "
...
for extraction_id, table_id, root_note, files, total, digest in [
    ("vpx-extraction.lotr-vpw-1-6", T_VPW, "vpxtool-extract", 1690, 276914856,
     "8852df0cf171e07b5d3bbeb32973b517f28e677cadde5f4f456b6fd705647f04"),
    ("vpx-extraction.lotr-jpsalas-600", T_JPS, "alt-jpsalas-600/vpxtool-extract", 991, 30053275,
     "a79e1c9ee71b1cbb70d013c3dc61ac135821826fe68ae83ec62f5f5eb3a0b781"),
    ("vpx-extraction.lotr-hanibal-4k", T_HAN, "alt-hanibal-4k/vpxtool-extract", 1782, 140122529,
     "6dda27475b77bcb21c29df93836fea64c725684e74c5864ff0191d233b7100ca"),
]:
    ...
    SOURCES.append({
        ...
        "locator": (
            f"Canonical manifest of the retained {root_note} tree produced with vpxtool git:0561bb4: every regular "
...
for table_id, script_id, author, subdir, name, sha, size, note in [
    ...
    (T_JPS, S_JPS, "JPSalas", "alt-jpsalas-600/", "JP's Lord of the Rings (Stern 2003) v600.vpx",
     ...
     "coordinate; only the Flasher props do."),
...
for address in range(1, 65):
    ...
    entry = {
        ...
        "id": device_id,
        "kind": "switch",
        "label": labelize(rec["name"]) if rec.get("name") else f"Unused Switch {address}",
...
for address in range(1, 33):
    ...
    if address == 25:
        notes.append(
            ...
            "address drives three physical flashlamps at the three pop bumpers. The single placement is the centroid "
            "of the three bumper positions."
        )
    entry = {
        "aliases": [{"namespace": "pinmame.solenoid", "value": str(address)},
                    {"namespace": "manual.address", "value": f"#{address}"}],
...
_recorded = {row["device"] for row in UNPLACED}
...
definition = {
    ...
    "coverage": {
        ...
        # (CORE_MODOUT_LAMP0 + 80/+88/+96, the last masked 0x07). Those nineteen emitters are
        # real, ROM-driven and on the playfield, and they are not enumerated here because
        # controllers/pinmame/whitestar.json caps pinmame.output.lamp at 1-80. Widening a shared
        # controller profile is a maintainer decision, so the gap is declared rather than hidden.
        ...
        "dimensions": {
            "address_enumeration": "observed", "catalog_identity": "validated", "mechanisms": "validated",
            "physical_wiring": "conflicted", "recreation_knowledge": "validated", "semantic_naming": "observed",
            "spatial_placement": "observed", "variant_coverage": "validated",
        },
        # The nineteen LED-board lamps are unenumerated and therefore also unnamed, so output
        # semantics is incomplete for the same reason output enumeration is. `polarity` and
        # `unresolved_conflicts` are switch 15, the one opto no retained recreation binds.
        "missing": [
            "output_enumeration", "output_semantics", "polarity", "spatial_placement", "unresolved_conflicts",
        ],
        "status": "partial",
    },
    "displays": displays,
...
# say the count.
_SUPPORT_WORDS = {1: "one_table", 2: "two_tables", 3: "three_tables"}
...
_wrongly_blamed = [row["id"] for row in observed_rows
                   ...
                   and len(row["supporting_tables"]) == 3]
if _wrongly_blamed:
    raise SystemExit(f"rows blamed on incomplete agreement that all three tables support: {_wrongly_blamed}")
...
# the artwork's doing.
_rejections_by_table: dict[str, int] = {"vpw-1-6": 0, "jpsalas-600": 0, "hanibal-4k": 0}
...
spatial_report = {
    ...
    "method": {
        ...
        "rule": (
            "A placement is validated only when all three retained tables agree on it within the threshold; "
            "anything supported by fewer is observed."
        ),
        "why_all_three": (
        ...
        "tables": {label: {"bounds": data["bounds"]} for label, data in consensus["tables"].items()},
    },
    "counts": {
        "validated_placements": len(validated_rows),
        "observed_placements": len(observed_rows),
        "single_table_placements": len(single_table_rows),
        "projected_placements": len(projection_rows),
        "computed_coordinates": len(computed_rows),
        "rejected_measurements": len(rejected_rows),
    },
    "blockers": [
        {
            "id": "led-board-520-5242-00-not-enumerated",
            "severity": "major",
            "summary": (
                "Nineteen playfield LEDs on board 520-5242-00 are driven by the ROM at public lamp addresses 81-99 "
                "and are not enumerated, so they carry no placement."
            ),
            "evidence": (
                "src/wpc/se.h names SE_BOARDID_520_5242_00 the 'Lord of The Rings 19 LED Board' and lotrGameData "
                "declares it. src/wpc/se.c writes CORE_MODOUT_LAMP0 + 80, + 88 and + 96 masked 0x07, and sets "
                "core_set_pwm_output_led_vfd(CORE_MODOUT_LAMP0 + 80, 3 * 8, CORE_MODOUT_LED, 4.f / 2.f) in the lotr-only branch. "
                "The retained VPW and JPSalas scripts both assign lamps 81-99."
            ),
            "why_not_fixed_here": (
                "controllers/pinmame/whitestar.json caps pinmame.output.lamp at 1-80. Widening a shared controller "
                "profile affects every Whitestar game and is a maintainer decision."
            ),
        },
        {
            ...
            "severity": "minor",
            "summary": (
                f"{len(observed_rows)} placements are observed rather than validated, for five distinct reasons: "
                f"{len(observed_reasons.get('incomplete_table_agreement', ()))} because fewer than all three retained "
                f"recreations agree on them, {len(observed_reasons.get('projection_never_promoted', ()))} because they "
                "are projections onto a co-located anchor and a projection is never promoted however well the tables "
                f"agree, {len(observed_reasons.get('computed_from_disagreeing_tables', ()))} because the coordinate is "
                "a per-axis median of tables that disagreed, so every contributing table appears in its source_refs "
                f"while none of them holds the published position, {len(observed_reasons.get('computed_within_one_table', ()))} "
                "because the coordinate is a centroid of co-located bodies inside a single table, which involves no "
                f"disagreement at all, and {len(observed_reasons.get('co_located_insert_pair', ()))} because no "
                "retained table separates the printed back-panel insert pairs, so the published coordinate is the pair "
                "rather than the bulb. reason_partition names which rows each reason covers; support_partition counts "
...
Coverage: **{definition['coverage']['status']} - manual-verified semantic I/O with full connector, wire-colour and driver-transistor wiring, mechanism inventory and behaviour, and normalized placements for the printed 8x10 lamp matrix and the switch matrix; held below author-ready because the nineteen LEDs of board 520-5242-00 at public lamps 81-99 cannot be enumerated while the shared Whitestar profile caps lamps at 1-80, and because switch 15's public opto polarity is unestablished**
...
Three recreations were retained and treated as three measurements. A placement is `validated` only when **all three agree** within {consensus['agreement_threshold']} normalized units; anything supported by fewer is `observed`, and {len(observed_rows)} placements are in that state against {len(validated_rows)} validated.
...
def _artifacts() -> tuple[tuple[Path, bytes], ...]:
	...
	All four are compared byte-for-byte. The spatial report and knowledge note are generated
	here rather than pinned by digest, so a statement cannot outlive the data behind it.
	"""
