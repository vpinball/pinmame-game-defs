...
'***********
  ...
  Sub Table_Init
	...
	With Controller
		...
		.ShowFrame = 0
		.HandleMechanics = 0
        If NOT CustomDMD Then .Hidden = 0				'hides the external DMD when in desktop mode and color ROM is not in use
		On Error Resume Next
		.Run GetPlayerHWnd
...
Sub WMKick(enabled)
    If enabled Then
		If sw10.ballcntover > 0 then
			KickBall KickerBall, 180, 55, 0, 0
...
Sub RightSlingShot_Slingshot
	...
	Me.TimerEnabled = 1    
	RubberRightSS1.Visible = 1
    RubberRightSS2.Visible = 0
    RStep = 0
End Sub
...
Sub RollingTimer_Timer()
	...
	For b = 0 to UBound(BOT)
		...
		If BOT(b).VelZ < -1 and BOT(b).z < 32 and BOT(b).z > 27 Then 'height adjust for ball drop sounds
				PlaySoundAt "fx_ball_drop" & b, BOT(b)
